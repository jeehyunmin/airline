...
public class WeekendMapperTest {

	MapDriver<LongWritable, Text, WeekendComplexKey, Text> map;
	...
	public void testRecord1() throws IOException {
		...
		map = MapDriver.newMapDriver(new WeekendDelayMapper());

		System.out.println(record1);
		map.withInput(new LongWritable(0), new Text(record1));
		map.withOutput(new WeekendComplexKey("IAD", 4), new Text("1,8"));
		map.runTest();
	}

	@Test
	public void testRecord19() throws IOException {
		
		map = MapDriver.newMapDriver(new WeekendDelayMapper());

		map.withInput(new LongWritable(0), new Text(record19));
		map.withOutput(new WeekendComplexKey("BNA", 1), new Text("7,-3"));
